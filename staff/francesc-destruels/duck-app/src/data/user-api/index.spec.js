...
describe('user api', () => {
    ...
    beforeEach(() => username = `frandei-${Math.random()}@gmail.com`)
    ...
    describe('update meu', () => {
        ...
        it('should succed on correct data', () => {
            ...
            let rename = { "name": "Emilio" }

            userApi.update(_id, token, rename)
                .then(response => {
                ...
                .then(response => {
                    const { status, data: { name: _name } } = response

                    expect(status).toBe('OK')
                    expect(_name).toBeDefined()
                    expect(_name).toBe(rename.name)
                })
        })
    })
})
