...
class Home extends Component {
    ...
    Promise.all([logic.retrieveDuck(id), logic.retrieveFavDucks()])
            .then(([{ id, title, imageUrl: image, description, price }, favs]) =>
                this.setState({ duck: { id, title, image, description, price }, favs})
            )
            .catch(error =>
                this.setState({ error: error.message })
            )
    ...
    render() {
        ...
        return <main>
            <h1>{hello} {name}!</h1>
            <button className="white" onClick={onLogout}>{logout}</button>
